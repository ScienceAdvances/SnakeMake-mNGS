...
def get_genome_prefix(config=config):
    """genome files prefix"""
    g=config["genome"]
    p="{0}/{1}/{1}_{2}_{3}_".format(g['dir'],g['species'].capitalize(),g['build'],g['release'])
    return p
...
def get_cram(wildcards):
    ...
    if u.all(axis=None):
        return expand(
            ["results/aligned/{{s}}{u}.aligned.cram"],
            u=u
        )
    else:
        return "results/aligned/{s}.aligned.cram"

def get_unaligned_fastq():
    """Get trimmed reads of given sample and unit."""
    if config["fastqs"].get("pe"):
        # paired-end sample
        return expand("results/aligned/{{s}}.{_}.fastq.gz",_=["1","2"])
    # single end sample
    return ["results/trimmed/{s}.fastq.gz"]

def get_unclassified_seq():
    """Get trimmed reads of given sample and unit."""
    if config["fastqs"].get("pe"):
        # paired-end sample
        return expand("results/kraken2/unclassified/{{s}}_{_}.fq",_=["1","2"])
    # single end sample
    return ["results/trimmed/{s}.fq"]
